...
def serial_connecting(): #! <<< ------ Serial Port Setting
    serialPort = serial.Serial('COM22', 115200, timeout=1)
    return serialPort
...
while(1):
    ...
    if Ser.inWaiting():
        ...
        if arduino == b'R': #!  ---------------------------       MODE == 0
            ...
            print('start sending image')  #!  <-----------------------------------------           MODE  == 2
            while arduino != b'g': #! @ <--- send 's' until ack 'g' back
                if Ser.inWaiting():
                    arduino = Ser.read()
                Ser.write(b's')
                time.sleep(0.1)
            print('arduino must be know prepairing for recieve DATA')
            arduino = ''
            time.sleep(1)
            Ser.flush()
            ...
            while arduino != b'g': #!    DATA 1
                if Ser.inWaiting():
                    arduino = Ser.read()
                Ser.write(storage[0])
                # print(storage[0],type(storage[0]))
                time.sleep(0.1)
